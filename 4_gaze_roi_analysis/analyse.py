...
with progress_instance as progress:
    ...
    progress.print("[bold yellow]Starting analysis for participant \"{}/{}\" at task {}".format(participant_id, video_id, starting_task))

    #### 1) Check ijk surfaces (calibration surfaces) on expected moments
    if(we_are_not_skipping_task(1, starting_task, progress, tasks)):
        check_calibration_surfaces(participant_id, video_id, '{}.json'.format(video_id), progress, tasks[0])
    ...
    if(we_are_not_skipping_task(2, starting_task, progress, tasks)):
        merge_gaze_positions(participant_id, video_id, progress, tasks[1])
